...
class LoanCalculatorApp:
    def __init__(self):
        ...
        Label(window, background='#F1D7DA', text='Number Of Years: ',
              font=("Helvetica", 13)).grid(
        ...
        Button(window, text='Compute Payment', command=self.compute_payment,
               font=("Helvetica", 12), bg="#4CAF50", fg="white").grid(
               column=1, row=5)

        self.result_monthly_payment = Label(window, background='#F1D7DA',
    ...
    def compute_payment(self):  # try and except (ValueError)
        ...
        self.result_total_payment.config(
            text=f'Total Payment: {total_payment:.3f}')
